...
`di_ppg` is also applicable to summarized data; just pass the counts to `success` and group size to `weight`:
```{r}
...
In cases where the proportion is used in calculating MOE, a proportion of 0 or 1 would lead to a zero MOE.  To account for these scenarios, the user could leverage the `prop_sub_0` and `prop_sub_1` parameters in `di_ppg` and `ppg_moe`  as substitutes.  These parameters default to `0.5`.
